                    ...
                    <div className="card" style={{ width: "18rem", flex: "0 0 auto" }} key={vehiculo.uid}>
                        ...
                        <div className="card-body">
                            ...
                            <input
                                    type="checkbox"
                                    className="checkbox"
                                    id={`Give-It-An-Id-${vehiculo.uid}`} // Usar el uid del vehiculo en el ID
                                    onClick={() => {
                                        const favoritoExistente = store.favoritos.find((favorito) => favorito === vehiculo.name);

                                        if (favoritoExistente) {
                                            actions.eliminarFavoritos(vehiculo.name);
                                            alert(`${vehiculo.name} has been removed from your favorites.`);
                                        } else {
                                            actions.añadirFavoritos(vehiculo.name);
                                            alert(`${vehiculo.name} has been added to your favorites. The force is with you ✨`);
                                        }
                                    }}
                                />
