...
export const CartaPlanetas = () => {
    ...
    const handleFav = (planeta) => {
        const favoritoExistente = store.favoritos.includes(planeta.name);

        if (favoritoExistente) {
            actions.eliminarFavoritos(planeta.name);
        } else {
                    ...
                    <div className="card" style={{ width: "18rem" }} key={planeta.uid}>
                        ...
                        <div className="card-body">
                            ...
                            <input
                                    type="checkbox"
                                    className="checkbox"
                                    id={`Give-It-An-Id-${planeta.uid}`}
                                    checked={store.favoritos.includes(planeta.name)}
                                    onChange={() => handleFav(planeta)}
                                />
                                <div className="svg-container">
                                    {store.favoritos.includes(planeta.name) ? (
